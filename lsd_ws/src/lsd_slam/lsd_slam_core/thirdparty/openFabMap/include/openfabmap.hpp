...
public:
  ...
  FabMap(const cv::Mat& clTree, double PzGe, double PzGNe, int flags, int numSamples = 0);
  virtual ~FabMap();
  ...
  void compare(const cv::Mat& queryImgDescriptor, std::vector<IMatch>& matches, bool addQuery = false,
               const cv::Mat& mask = cv::Mat());
  void compare(const cv::Mat& queryImgDescriptor, const cv::Mat& testImgDescriptors, std::vector<IMatch>& matches,
               const cv::Mat& mask = cv::Mat());
  void compare(const cv::Mat& queryImgDescriptor, const std::vector<cv::Mat>& testImgDescriptors,
               std::vector<IMatch>& matches, const cv::Mat& mask = cv::Mat());
  void compare(const std::vector<cv::Mat>& queryImgDescriptors, std::vector<IMatch>& matches, bool addQuery = false,
               const cv::Mat& mask = cv::Mat());
  void compare(const std::vector<cv::Mat>& queryImgDescriptors, const std::vector<cv::Mat>& testImgDescriptors,
               std::vector<IMatch>& matches, const cv::Mat& mask = cv::Mat());
...
protected:
  ...
  // method.
  virtual void getLikelihoods(const cv::Mat& queryImgDescriptor, const std::vector<cv::Mat>& testImgDescriptors,
...
{
public:
  FabMap1(const cv::Mat& clTree, double PzGe, double PzGNe, int flags, int numSamples = 0);
  virtual ~FabMap1();

protected:
  // FabMap1 implementation of likelihood comparison
  void getLikelihoods(const cv::Mat& queryImgDescriptor, const std::vector<cv::Mat>& testImgDescriptors,
                      std::vector<IMatch>& matches);
};
...
{
public:
  FabMapLUT(const cv::Mat& clTree, double PzGe, double PzGNe, int flags, int numSamples = 0, int precision = 6);
  virtual ~FabMapLUT();
...
{
public:
  FabMapFBO(const cv::Mat& clTree, double PzGe, double PzGNe, int flags, int numSamples = 0,
...
protected:
  ...
  void getIndexLikelihoods(const cv::Mat& queryImgDescriptor, std::vector<double>& defaults,
                           std::map<int, std::vector<int> >& invertedMap, std::vector<IMatch>& matches);
  void addToIndex(const cv::Mat& queryImgDescriptor, std::vector<double>& defaults,
...
private:
  ...
  void recAddToTree(cv::Mat& cltree, int q, int pq, std::list<info>& remaining_edges);
