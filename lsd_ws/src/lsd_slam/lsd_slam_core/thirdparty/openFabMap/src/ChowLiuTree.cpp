...
Mat ChowLiuTree::make(double infoThreshold)
{
  ...
  mergedImgDescriptors = cv::Mat(descCount, imgDescriptors[0].cols, imgDescriptors[0].type());
...
double ChowLiuTree::P(int a, bool za)
{
  if (za)
  {
    return (0.98 * cv::countNonZero(mergedImgDescriptors.col(a)) / mergedImgDescriptors.rows) + 0.01;
  }
  else
  {
    return 1 - ((0.98 * cv::countNonZero(mergedImgDescriptors.col(a)) / mergedImgDescriptors.rows) + 0.01);
  }
}
...
void ChowLiuTree::recAddToTree(cv::Mat& cltree, int q, int pq, list<info>& remaining_edges)
{
...
double ChowLiuTree::calcMutInfo(int word1, int word2)
{
  ...
  if (P00)
    accumulation += P00 * log(P00 / (P(word1, false) * P(word2, false)));

  double P01 = JP(word1, false, word2, true);
  if (P01)
    accumulation += P01 * log(P01 / (P(word1, false) * P(word2, true)));

  double P10 = JP(word1, true, word2, false);
  if (P10)
    accumulation += P10 * log(P10 / (P(word1, true) * P(word2, false)));

  double P11 = JP(word1, true, word2, true);
  if (P11)
    accumulation += P11 * log(P11 / (P(word1, true) * P(word2, true)));
