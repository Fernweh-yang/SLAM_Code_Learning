...
namespace of2
{
FabMap::FabMap(const Mat& _clTree, double _PzGe, double _PzGNe, int _flags, int _numSamples)
  : clTree(_clTree), PzGe(_PzGe), PzGNe(_PzGNe), flags(_flags), numSamples(_numSamples)
{
...
void FabMap::compare(const Mat& queryImgDescriptor, vector<IMatch>& matches, bool addQuery, const Mat& mask)
{
  CV_Assert(!queryImgDescriptor.empty());
  vector<Mat> queryImgDescriptors;
  for (int i = 0; i < queryImgDescriptor.rows; i++)
...
void FabMap::compare(const Mat& queryImgDescriptor, const Mat& testImgDescriptor, vector<IMatch>& matches,
                     const Mat& mask)
{
  CV_Assert(!queryImgDescriptor.empty());
  vector<Mat> queryImgDescriptors;
...
void FabMap::compare(const Mat& queryImgDescriptor, const vector<Mat>& testImgDescriptors, vector<IMatch>& matches,
                     const Mat& mask)
{
  CV_Assert(!queryImgDescriptor.empty());
  vector<Mat> queryImgDescriptors;
...
void FabMap::compare(const vector<Mat>& queryImgDescriptors, vector<IMatch>& matches, bool addQuery, const Mat& mask)
{
  ...
  for (size_t i = 0; i < queryImgDescriptors.size(); i++)
  {
    ...
    compareImgDescriptor(queryImgDescriptors[i], i, testImgDescriptors, matches);
    if (addQuery)
      add(queryImgDescriptors[i]);
  }
}

void FabMap::compare(const vector<Mat>& queryImgDescriptors, const vector<Mat>& testImgDescriptors,
...
void FabMap::compareImgDescriptor(const Mat& queryImgDescriptor, int queryIndex, const vector<Mat>& testImgDescriptors,
...
void FabMap::getLikelihoods(const Mat& queryImgDescriptor, const vector<Mat>& testImgDescriptors,
                            vector<IMatch>& matches)
{
}
  ...
  if (flags & MEAN_FIELD)
  {
    ...
    if (flags & NAIVE_BAYES)
    {
      for (int q = 0; q < clTree.cols; q++)
      {
        ...
        logP += log(Pzq(q, false) * PzqGeq(zq, false) + Pzq(q, true) * PzqGeq(zq, true));
      }
    }
    else
    {
...
void FabMap::normaliseDistribution(vector<IMatch>& matches)
{
  ...
  if (flags & MOTION_MODEL)
  {
    ...
    if (priorMatches.size() > 2)
    {
      ...
      matches[1].match += log(
          (2 * (1 - mBias) * priorMatches[1].match + priorMatches[1].match + 2 * mBias * priorMatches[2].match) / 3);
      for (size_t i = 2; i < priorMatches.size() - 1; i++)
      {
        matches[i].match = matches[i].likelihood;
        matches[i].match += log((2 * (1 - mBias) * priorMatches[i - 1].match + priorMatches[i].match +
                                 2 * mBias * priorMatches[i + 1].match) /
                                3);
      }
      matches[priorMatches.size() - 1].match = matches[priorMatches.size() - 1].likelihood;
      matches[priorMatches.size() - 1].match +=
          log((2 * (1 - mBias) * priorMatches[priorMatches.size() - 2].match +
               priorMatches[priorMatches.size() - 1].match + 2 * mBias * priorMatches[priorMatches.size() - 1].match) /
              3);
    ...
    for (size_t i = 0; i < matches.size(); i++)
    {
      matches[i].match = sFactor * matches[i].match + (1 - sFactor) / matches.size();
    }
...
{
  return PeqGL(q, Lzq, false) * PzqGeq(zq, false) + PeqGL(q, Lzq, true) * PzqGeq(zq, true);
}
...
void FabMap1::getLikelihoods(const Mat& queryImgDescriptor, const vector<Mat>& testImgDescriptors,
                             vector<IMatch>& matches)
{
  for (size_t i = 0; i < testImgDescriptors.size(); i++)
  {
...
FabMapLUT::FabMapLUT(const Mat& _clTree, double _PzGe, double _PzGNe, int _flags, int _numSamples, int _precision)
...
void FabMapLUT::getLikelihoods(const Mat& queryImgDescriptor, const vector<Mat>& testImgDescriptors,
                               vector<IMatch>& matches)
{
  double precFactor = (double)pow(10.0, -precision);
  ...
  for (size_t i = 0; i < testImgDescriptors.size(); i++)
  {
    ...
    for (int q = 0; q < clTree.cols; q++)
    {
      logP += table[q][(queryImgDescriptor.at<float>(0, pq(q)) > 0) + ((queryImgDescriptor.at<float>(0, q) > 0) << 1) +
...
FabMapFBO::FabMapFBO(const Mat& _clTree, double _PzGe, double _PzGNe, int _flags, int _numSamples,
                     double _rejectionThreshold, double _PsGd, int _bisectionStart, int _bisectionIts)
  : FabMap(_clTree, _PzGe, _PzGNe, _flags, _numSamples)
  , PsGd(_PsGd)
  , rejectionThreshold(_rejectionThreshold)
  , bisectionStart(_bisectionStart)
...
void FabMapFBO::getLikelihoods(const Mat& queryImgDescriptor, const vector<Mat>& testImgDescriptors,
                               vector<IMatch>& matches)
{
  ...
  for (multiset<WordStats>::iterator wordIter = wordData.begin(); wordIter != wordData.end(); wordIter++)
  {
    ...
    {
      bool Lzq = testImgDescriptors[matchIndices[i]].at<float>(0, wordIter->q) > 0;
      queryMatches[matchIndices[i]].likelihood += log((this->*PzGL)(wordIter->q, zq, zpq, Lzq));
...
void FabMapFBO::setWordStatistics(const Mat& queryImgDescriptor, multiset<WordStats>& wordData)
{
  ...
  {
    wordData.insert(
        WordStats(q, PzqGzpq(q, queryImgDescriptor.at<float>(0, q) > 0, queryImgDescriptor.at<float>(0, pq(q)) > 0)));
  }

  double d = 0, V = 0, M = 0;
  bool zq, zpq;

  for (multiset<WordStats>::reverse_iterator wordIter = wordData.rbegin(); wordIter != wordData.rend(); wordIter++)
  {
    zq = queryImgDescriptor.at<float>(0, wordIter->q) > 0;
    zpq = queryImgDescriptor.at<float>(0, pq(wordIter->q)) > 0;

    d = log((this->*PzGL)(wordIter->q, zq, zpq, true)) - log((this->*PzGL)(wordIter->q, zq, zpq, false));

    V += pow(d, 2.0) * 2 * (Pzq(wordIter->q, true) - pow(Pzq(wordIter->q, true), 2.0));
    M = std::max(M, fabs(d));
...
FabMap2::FabMap2(const Mat& _clTree, double _PzGe, double _PzGNe, int _flags) : FabMap(_clTree, _PzGe, _PzGNe, _flags)
{
  ...
  for (int q = 0; q < clTree.cols; q++)
  {
    d1.push_back(log((this->*PzGL)(q, false, false, true) / (this->*PzGL)(q, false, false, false)));
    d2.push_back(log((this->*PzGL)(q, false, true, true) / (this->*PzGL)(q, false, true, false)) - d1[q]);
    d3.push_back(log((this->*PzGL)(q, true, false, true) / (this->*PzGL)(q, true, false, false)) - d1[q]);
    d4.push_back(log((this->*PzGL)(q, true, true, true) / (this->*PzGL)(q, true, true, false)) - d1[q]);
...
void FabMap2::getLikelihoods(const Mat& queryImgDescriptor, const vector<Mat>& testImgDescriptors,
                             vector<IMatch>& matches)
{
  if (&testImgDescriptors == &(this->testImgDescriptors))
  {
...
void FabMap2::addToIndex(const Mat& queryImgDescriptor, vector<double>& defaults, map<int, vector<int> >& invertedMap)
{
...
void FabMap2::getIndexLikelihoods(const Mat& queryImgDescriptor, vector<double>& defaults,
                                  map<int, vector<int> >& invertedMap, vector<IMatch>& matches)
{
