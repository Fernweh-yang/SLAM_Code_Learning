...
namespace lsd_slam
{
    ...
    */
    ...
    void Frame::initialize(int id, int width, int height, const Eigen::Matrix3f &K, double timestamp)
    {
        ...
        {   
            // *************** 初始化金字塔 ***************
            // 这些金字塔主要用于traking时的初始化，目的是用更少的关键点去计算一个更好的初始化，见论文3.5的coarse to fine approach
    ...
    void Frame::buildImage(int level)
    {
        ...
        // 递归，requeire()函数中会判断当前是在build image、gradients还是depthandcariance。比如这里是image，所以还是调用buildImage()
        // 从底层开始构建金字塔，因为上层金字塔是以底层为基础的
        require(IMAGE, level - 1);
        // 调用buildMutex互斥锁
        boost::unique_lock<boost::mutex> lock2(buildMutex);
        // 如果该层金字塔是否已经构建完成，直接返回
    ...
    void Frame::buildGradients(int level)
    {
        ...
        int width = data.width[level];      // 图像宽
        int height = data.height[level];    // 图像高
        // 分配梯度的内存,之后的w x h的图像存储都是一维的形式
