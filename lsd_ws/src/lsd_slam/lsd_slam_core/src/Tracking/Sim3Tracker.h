...
public:
  ...
  Sim3 trackFrameSim3(TrackingReference* reference, Frame* frame, const Sim3& frameToReference_initialEstimate,
...
private:
  void calcSim3Buffers(const TrackingReference* reference, Frame* frame, const Sim3& referenceToFrame, int level,
                       bool plotWeights = false);
#if defined(ENABLE_SSE)
  void calcSim3BuffersSSE(const TrackingReference* reference, Frame* frame, const Sim3& referenceToFrame, int level,
                          bool plotWeights = false);
#endif
#if defined(ENABLE_NEON)
  void calcSim3BuffersNEON(const TrackingReference* reference, Frame* frame, const Sim3& referenceToFrame, int level,
                           bool plotWeights = false);
#endif

  Sim3ResidualStruct calcSim3WeightsAndResidual(const Sim3& referenceToFrame);
#if defined(ENABLE_SSE)
  Sim3ResidualStruct calcSim3WeightsAndResidualSSE(const Sim3& referenceToFrame);
#endif
#if defined(ENABLE_NEON)
  Sim3ResidualStruct calcSim3WeightsAndResidualNEON(const Sim3& referenceToFrame);
#endif
