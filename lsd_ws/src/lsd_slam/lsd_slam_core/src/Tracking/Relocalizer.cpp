...
namespace lsd_slam
{
    ...
    void Relocalizer::updateCurrentFrame(std::shared_ptr<Frame> currentFrame)
    {
        ...
        // Relocalizer类初始化时，hasResult为false
        if (hasResult)
            return;
    ...
    void Relocalizer::getResult(Frame *&out_keyframe, std::shared_ptr<Frame> &frame, int &out_successfulFrameID,
    ...
    void Relocalizer::threadLoop(int idx)
    {
        ...
        while (continueRunning)
        {
            ...
            if (nextRelocIDX < maxRelocIDX && CurrentRelocFrame)
            {
                ...
                // try neighbours
                float todoGoodVal =
                    tracker->pointUsage * tracker->lastGoodCount / (tracker->lastGoodCount + tracker->lastBadCount);
                if (todoGoodVal > relocalizationTH)
                {
                    ...
                    if (numGoodNeighbours > numBadNeighbours || numGoodNeighbours >= 5)
                    {
                        if (enablePrintDebugInfo && printRelocalizationInfo)
                            printf("RELOCALIZED! frame %d on %d (bestNeighbour %d): good %2.1f%%, usage %2.1f%%, GoodNeighbours %d / "
                                   "%d\n",
                                   myRelocFrame->id(), todo->id(), bestKF->id(), 100 * bestNeightbourGoodVal, 100 * bestNeighbourUsage,
                    ...
                    {
                        if (enablePrintDebugInfo && printRelocalizationInfo)
                            printf("FAILED RELOCALIZE! frame %d on %d (bestNeighbour %d): good %2.1f%%, usage %2.1f%%, GoodNeighbours "
                                   "%d / %d\n",
                                   myRelocFrame->id(), todo->id(), bestKF->id(), 100 * bestNeightbourGoodVal, 100 * bestNeighbourUsage,
                                   numGoodNeighbours, numGoodNeighbours + numBadNeighbours);
                    }
